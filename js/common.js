...
window.onload = function() {
    com.bg = new com.img.Bg();
    com.dot = new com.img.Dot();
    com.pane = new com.img.Pane();
...
com.createMans = function(map) {
    for (var i = 0; i < map.length; i++) {
        for (var n = 0; n < map[i].length; n++) {
            var key = map[i][n];
            if (key) {
                com.mans[key] = new com.img.Man(key);
                com.mans[key].x = n;
                com.mans[key].y = i;
...
//黑子为红字价值位置的倒置
com.value.C = com.arrClone(com.value.c).reverse();
com.value.M = com.arrClone(com.value.m).reverse();
com.value.X = com.value.x;
com.value.S = com.value.s;
com.value.J = com.value.j;
com.value.P = com.arrClone(com.value.p).reverse();
com.value.Z = com.arrClone(com.value.z).reverse();
...
com.img = {} //图片
com.img.Man = function(key, x, y) {
    this.pater = key.slice(0, 1); //从第1个到第二个
    ...
    this.show = function() {
        if (this.isShow) {
            ...
            // 将从栈顶部弹出的一组状态信息，并根据此恢复当前绘图环境的各个状态
            com.ct.restore();
        }
    }
